...
export function LocalProductDetail({ id }: { id: string }) {
  ...
  return <>
    ...
    <section className="product-layout">
      ...
      <div><h1 style={{ fontSize: 38 }}>{product.name}</h1><p style={{ color: "#737373" }}>SKU: {product.sku} | Marca: {product.brand}</p><div className="buy-box"><ProductPurchase product={product} /></div><div className="card" style={{ marginTop: 18 }}><h3><Truck size={18} /> Calcular envío</h3><p>Ingresá tu código postal al finalizar la compra o consultá por WhatsApp.</p><a className="button-muted" href="https://wa.me/5492490000000"><MessageCircle size={18} /> Consultar disponibilidad</a></div></div>
    </section>
